...
{
	std::string game_table_name = "Standard Table";
	GameTable* active_game_table = new GameTable(game_table_name);
	this->active_game_table = active_game_table; // TODO -  Set to nulptr and create using main menu bar. 
...
void ApplicationHandler::renderMainMenuBar()
{
	if (ImGui::BeginMainMenuBar()) {
		...
		if (this->active_game_table != nullptr) {
			this->active_game_table->renderMenu();
		}
...
{
	if (this->active_game_table != nullptr) {
		this->active_game_table->renderActiveBoard();
	}
	else {
		std::cout << "No GameTable Opened!!" << std::endl;
	}
}
...
//	GameTable game_table;
//	recent_game_tables.insert({ game_table, game_table });
//}
//
