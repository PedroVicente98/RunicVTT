...
std::filesystem::path NotesManager::defaultSavePath(const Note& n) const
{
    ...
    if (n.table_name)
        return cfg_.gameTablesRootDir / *n.table_name / "Notes" / fname;
...
bool NotesManager::parseMarkdownFile(const std::filesystem::path& path, Note& outNote)
{
    ...
    if (meta.count("creation_ts"))
    {
        try
        {
            n.creation_ts = fromEpochMillis(std::stoll(meta["creation_ts"]));
        }
        catch (...)
        {
            n.creation_ts = Clock::now();
    ...
    if (meta.count("last_update_ts"))
    {
        try
        {
            n.last_update_ts = fromEpochMillis(std::stoll(meta["last_update_ts"]));
        }
        catch (...)
        {
            n.last_update_ts = n.creation_ts;
    ...
    if (meta.count("table") && !meta["table"].empty())
        n.table_name = meta["table"];
    if (meta.count("shared"))
        n.shared = (meta["shared"] == "1");
    if (meta.count("shared_from") && !meta["shared_from"].empty())
        n.shared_from = meta["shared_from"];
...
{
    if (toaster_)
        toaster_->Push(ImGuiToaster::Level::Info, msg);
}
void NotesManager::toastGood(const std::string& msg) const
{
    if (toaster_)
        toaster_->Push(ImGuiToaster::Level::Good, msg);
}
void NotesManager::toastWarn(const std::string& msg) const
{
    if (toaster_)
        toaster_->Push(ImGuiToaster::Level::Warning, msg);
}
void NotesManager::toastError(const std::string& msg) const
{
    if (toaster_)
        toaster_->Push(ImGuiToaster::Level::Error, msg);
}
...
std::string NotesManager::resolveRef(const std::string& ref) const
{
    ...
    if (looksLikeShortHex_(ref))
    {
        ...
        for (const auto& kv : notesByUuid_)
        {
            ...
            for (char c : uuid)
                if (c != '-')
                    compact.push_back((char)std::tolower((unsigned char)c));

            std::string needle = toLower_(ref);
            if (compact.rfind(needle, 0) == 0)
            { // prefix match
                if (matchUuid.empty())
                    matchUuid = uuid;
                else
...
{
    if (s.size() < 8)
        return false; // minimum 8 for usefulness
    for (char c : s)
    {
        if (c == '-')
            return false;
